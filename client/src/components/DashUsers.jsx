...
export default function DashUsers() {
  ...
  useEffect(()=>{
    const fetchUsers = async () => {
      try {
        const res =await fetch(`${__API_BASE__}/user/getusers`)
  ...
  const handleShowMore= async()=>{
    ...
    try {
      const res = await fetch(`${__API_BASE__}/user/getusers?startIndex=${startIndex}`);
      const data = await res.json();
      if(res.ok){
  ...
  const handleDeleteUser = async()=>{
    try {
      const res = await fetch(`/${__API_BASE__}/user/delete/${userIdToDelete} `,{
        method:'DELETE',
      });
      const data = await res.json();
      if(res.ok){
      ...
      <Modal show={showModal} onClose={()=> setShowModal(false)} popup 
            ...
            <Modal.Body>
              <div className='text-center'>
                ...
                <h3 className='mb-5 text-lg text-gray-500 dark:text-gray-400'>Are you sure want you to delete this user ?</h3>
