...
export default function PostPage() {
    ...
    useEffect(()=>{
        const fetchPost = async () =>{
            try {
                setLoading(true);
                const res = await fetch(`${__API_BASE__}/post/getposts?slug=${postSlug}`);
    ...
    useEffect(()=>{
      try {
        const fetchRecentPosts = async () => {
            const res =await fetch(`${__API_BASE__}/post/getposts?limit=3`);
            const data = await res.json();
            if(res.ok){
  ...
  return (
   <main className='p-3 flex flex-col max-w-6xl mx-auto min-h-screen'>
    ...
    <img src={post && post.image} alt={post && post.title} className='mt-10
     ...
     <div className='flex justify-center p-3 border-b border-slate-500 mx-auto
